...
for (let i = 0; i < buttonEl.length; i++) {
    buttonEl[i].addEventListener('click', handleClick);
}
...
function handleClick(clickedButtonEvent) {
    ...
    let textButtonEl = card.button.innerText;

    if (textButtonEl == 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textButtonEl == 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let anyText = 'Lorem ipsum dolor sit amet consectetur adipisicing elit. Sapiente a officia, hic aliquam architecto placeat velit laborum eaque itaque asperiores ratione, eligendi nam dignissimos? Soluta ratione reiciendis ad odio dolorum?';
